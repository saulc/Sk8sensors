    ...
    difference(){  
       ...
       translate([0, 0, mcon/2 ]) rotate([90,0,0]) cy(mcon,l+w,  fn/1.25); 
         hull()  {
             ...
             for(i=[-1,1]) translate([0, i*l/2-w*i, w/2 ]) rotate([0,90,0]) cy(w,w+2,  fn); 
             }
     }
     //mostly for long boards i guess?
//        for(j=[0,1]) mirror([j, 0, 0])   for(i=[-1,1]) translate([w/2, i*l*.22, conh-con/2-x/2 ]) wb(x);
        }
 }
 
module wb(x){
    hull(){
       translate([0, 0, 0 ]) rotate([0,90,0]) cy(con+x,2,  fn); 
       translate([-w/3, 0, con/8+x*2.2  ]) rotate([0,90,0]) cy(con/2+x,2,  fn); 
    }
}
